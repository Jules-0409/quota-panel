...
调用失败就如实显示错误，不会拿旧数字冒充。自动每 5 分钟一轮（可在 ⚙ 里改），↻ / 托盘菜单可立即刷新；
...
- **开机自启**：把 `/Applications/quota-panel.app` 加进「系统设置 → 通用 → 登录项」。
  它是菜单栏程序，不会占 Dock 图标。

## 配置

设置面板里三个数字，**点一下立刻生效并写盘**（没有「保存」按钮）：

| 项 | 默认 | 范围 | 作用 |
| --- | --- | --- | --- |
| 自动刷新间隔 | 5 分钟 | 1–60 | 后台每隔多久取一次数 |
| 警告阈值 | 70% | 1–99 | 达到就转黄 |
| 危险阈值 | 90% | 2–100 | 达到就转红，必须严格高于警告阈值 |
...
命令行版本（`quota`）不吃这份配置：它每次都用默认阈值上色。
