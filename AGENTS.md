...
桌面常驻的 AI 额度监控小组件（Tauri v2 + Rust）。**项目档位：正式项目**——
已经开源发布、会被别人 clone 去用，所以按母版的「正式必须」标准走。
...
- **红线仍是母版的第二节**（`~/.factory/AGENTS.md`）：不碰代理、不提交密钥、
  公司代码不推个人 GitHub。本文件不能覆盖红线。
- **这个仓库在 Jules 的个人 GitHub 上**（`Jules-0409/quota-panel`），
  公开仓库，**任何提交都等于对外发布**。凭据、内网地址、公司业务数据一律不进仓库。
- **push 需要当次明确授权**。母版说：授权一次不等于以后都能推。
...
**改了本文件，要在汇报里说明**（母版第六节）。项目规则冲突时以本文件为准，
唯独母版第二节红线永远最高。
